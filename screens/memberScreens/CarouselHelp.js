...
export default class CarouselHelp extends Component {
	...
	componentDidMount() {
		...
		var tab = [];
		axiosNoTokenInstance.get("/contributions").then((response) => {
	...
	_renderItem({ item, index }) {
		return (
			...
			>
				...
				{item.state === 1 ? (
					<View style={{ backgroundColor: "white", height: 1, width: 240, marginBottom: 12, marginLeft: 20 }}></View>
				) : (
					<View style={{ backgroundColor: "transparent", height: 40, width: 440 }}></View>
				)}
				{item.state === 1 ? <Text style={styles.text}>Contribution : {this.getContribution(item)}</Text> : null}

				<NameComponent members={this.state.members} item={item} />
				<Text numberOfLines={2} style={styles.text}>
					Titre : {this.getHelpTypeName(item)}
				</Text>
				<View style={{ marginBottom: 10 }}>
					<Animatable.View animation="bounceIn" duration={4500}>
						<Button label="Details" labelIcon="eye" style={{ marginRight: "10" }} loading={false} />
					</Animatable.View>
				</View>
			</View>
		);
