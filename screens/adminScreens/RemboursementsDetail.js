...
const RemboursementsDetail = (props) => {
	...
	const refunds = props.route.params.refunds;
	...
	return (
		<View style={globalStyles.container}>
			{refunds ? (
					...
					<FlatList
						...
						renderItem={({ item }) => (
							<ListItem bottomDivider containerStyle={{ borderRadius: 20, marginBottom: 20 }}>
								<ListItem.Content>
									<ListItem.Title>{`Remboursement du membre : ${getMemberNames(item.member_id)}`}</ListItem.Title>
									<ListItem.Subtitle>{`montant total de remboursement: ${item.amount}`}</ListItem.Subtitle>
								</ListItem.Content>
							</ListItem>
						)}
					/>
				...
				<Text>Aucun Remboursement faite</Text>
			)}
		</View>
	);
};

export default RemboursementsDetail;
