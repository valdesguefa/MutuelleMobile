...
const NameComponent = (props) => {
	...
	useEffect(() => {
		//  console.log('jkfjdskfjdkfjdkfjdkjfkd', props.members)
		setitem(props.item);
		...
		const getNameMember = () => {
			...
			for (let obj1 of members) {
				if (item.member_id === obj1.id) {
					console.log("jkfjdskfjdkfjdkfjdkjfkd", URL + `/users/${obj1.user_id}/`);
					axios
	...
	return (
		<Text numberOfLines={2} style={styles.text}>
			Concerne : <Text style={styles.textResult}>{member}</Text>
		</Text>
	);
};
		...
		return (
			...
			>
				<Text style={styles.text1}>
					Montant : <Text style={styles.textResult}>{item.amount}</Text>
				</Text>
				<Text style={styles.text}>
					Contribution : <Text style={styles.textResult}>{item.unit_amount} XAF / membre</Text>
				</Text>
				{/*
                    item.state === 1 ? <View style={{ backgroundColor: 'white', height: 1, width: 260, alignSelf: 'center', marginTop: 12, marginBottom: 12}}></View>
                        : <View style={{ backgroundColor: 'transparent', height: 30, width: 440 }}></View>
                */}
				<View
					style={{
				...
				{item.state === 1 ? (
					<Text style={styles.text}>
						Contribution : <Text style={styles.textResult}>{this.getContribution(item)}</Text>
					</Text>
				) : null}

				<NameComponent members={this.state.members} item={item} />
				<Text numberOfLines={2} style={styles.text}>
					Titre : <Text style={styles.textResult}>{this.getHelpTypeName(item)}</Text>
				</Text>
				<View style={{ marginBottom: 10 }}>
					<Animatable.View animation="bounceIn" duration={4500}>
						<Button
							label="Details"
							onPress={() => this.props.navigation.navigate("helpDetail", item)}
							labelIcon="eye"
			...
			return (
				<SafeAreaView style={{ flex: 1, paddingTop: 20 }}>
					<View>
						...
						/>
						<View style={{ marginTop: 260, alignSelf: "center", position: "absolute" }}>{this.pagination}</View>
					</View>
				</SafeAreaView>
			);
		} else {
