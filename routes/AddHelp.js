        ...
        */
		...
		axios
			...
			.then((response) => {
				...
				for (let obj6 of members.filter((item) => item.id !== concern["id"])) {
					console.log("sjkfkhsjdhskjhdksjdk", {
						help_id: response.data.id,
						date: limitDate,
						state: 0,
						member_id: obj6["id"],
						administrator_id: 3,
			...
			<View style={{ marginTop: 30 }}>
				<View style={{ flexDirection: "column", paddingTop: 0, marginVertical: 40 }}>
					<ModalDropdown
						...
						renderItem={(item) => (
							<View
								style={{ height: 40, width: Dimensions.get("window").width * 0.84, borderWidth: 0.5, borderRadius: 5 }}
							>
								<View style={{ flexDirection: "row" }}>
									<Text>
										{" "}
										<Text>{item.title}</Text> - <Text>{item.amount}</Text>{" "}
									</Text>
								</View>
							</View>
						)}
					>
						...
						>
							{helpTypeTitle.hasOwnProperty("id") ? (
								<Text style={{ marginLeft: Dimensions.get("window").width * 0.1, marginTop: 15 }}>
									{helpTypeTitle.title} - {helpTypeTitle.amount}
								</Text>
							) : (
				...
				<View style={{ flexDirection: "column", paddingTop: 0, marginVertical: 5 }}>
					...
					>
						...
						>
							{concern.hasOwnProperty("id") ? (
								<Text style={{ marginLeft: Dimensions.get("window").width * 0.25, marginTop: 15 }}>
									{getNameMember(concern.id).name}
								</Text>
							) : (
								<Text style={{ marginLeft: Dimensions.get("window").width * 0.17, marginTop: 15 }}>
									Membre concerne par l'aide
								</Text>
							)}
						</View>
					</ModalDropdown>
